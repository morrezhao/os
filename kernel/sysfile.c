...
sys_dup3(void)
{
  ...
  if (argfd(0, &old_fd, &f) < 0 || argint(1, &new_fd) < 0 || new_fd < 0 || new_fd > NOFILE) {
    return -1;
  }
...
sys_getcwd(void)
{
    ...
    *s = '\0';
    ...
    if (addr == NULL) {  // 使用栈空间存储路径
        struct proc *p = myproc();
        if (p->trapframe->sp < path_length)
            return NULL;  // 栈空间不足
        p->trapframe->sp -= path_length;
        addr = p->trapframe->sp;
    } else if (size < path_length) {
        return NULL;  // 用户缓冲区不足
    }

    // 拷贝路径到用户空间
    if (copyout2(addr, s, path_length) < 0)
        return NULL;
